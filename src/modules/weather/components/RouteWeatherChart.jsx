...
// Styles constants pour éviter la recréation à chaque render
const styles = {
  title: {
    fontSize: '18px', 
    fontWeight: '600', 
...
// Composant mémorisé pour les boutons de type de carte
const ChartTypeButton = memo(({ type, currentType, onClick, icon: Icon, label }) => {
...
// Composant mémorisé pour le SVG de la carte
const ChartSVG = memo(({ chartType, selectedRegion, selectedLevel }) => {
  return (
...
export const MeteoCharts = memo(({ selectedAltitude = 0 }) => {
  ...
  // Mémorisation des données statiques
  const flightLevels = useMemo(() => [
  ...
  }, [selectedAltitude, chartType, flightLevels]);

  // Charger la carte avec useCallback pour éviter la recréation
  ...
  }, [chartUrl, chartType, selectedRegion, selectedLevel]);

  // Mémoriser les styles de transformation
  ...
  return (
    <div style={{ marginTop: '24px' }}>
      ...
      <div style={{ display: 'flex', gap: '16px', marginBottom: '20px', flexWrap: 'wrap' }}>
        ...
        {chartType === 'WINTEM' && (
          <div style={{ flex: '1', minWidth: '200px' }}>
            <label style={styles.label}>Niveau de vol</label>
            <select
              value={selectedLevel}
      ...
      <div style={{ backgroundColor: 'white', borderRadius: '8px', border: '1px solid #e5e7eb', overflow: 'hidden' }}>
        ...
        <div style={styles.toolbar}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            {/* Zoom */}
            <button onClick={() => handleZoom(-10)} style={styles.toolbarButton} title="Zoom arrière">
              <ZoomOut size={16} />
            </button>
            <span style={{ padding: '6px 12px', backgroundColor: '#f3f4f6', borderRadius: '6px', fontSize: '13px', minWidth: '50px', textAlign: 'center' }}>
              {scale}%
            </span>
            <button onClick={() => handleZoom(10)} style={styles.toolbarButton} title="Zoom avant">
              <ZoomIn size={16} />
            </button>
            ...
            <button onClick={handleDownload} style={styles.toolbarButton} title="Télécharger">
              <Download size={16} />
            </button>
          </div>
        </div>
        ...
        <div style={{
          ...
          {loading ? (
            ...
            <div style={transformStyle}>
              <ChartSVG chartType={chartType} selectedRegion={selectedRegion} selectedLevel={selectedLevel} />
            </div>
          ) : (
