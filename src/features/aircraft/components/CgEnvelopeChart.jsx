...
const CGEnvelopeChart = memo(({ cgEnvelope, massUnit = 'kg' }) => {
  ...
  // Points arrière — 1 CG partagé + 2 masses (limite arrière verticale)
  ...
  // 2. Point Aft Max (haut droit) — utilise le CG arrière partagé
  if (aftMaxWeight > 0 && aftCG > 0) {
    envelopePoints.push({ w: aftMaxWeight, cg: aftCG, label: 'Aft Max' });
  }

  // 3. Point Aft Min (bas droit) — même CG, masse différente
