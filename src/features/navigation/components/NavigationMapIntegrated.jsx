  ...
  return (
    <div>
      ...
      <div style={{ position: 'relative' }}>
        ...
        {/* Statistiques de vol (en bas à gauche) */}
        {flightStats && (
          <div style={{
            position: 'absolute',
            bottom: '16px',
            ...
            <div style={{ fontSize: '12px', display: 'grid', gap: '4px' }}>
              <div>📏 Distance: <strong>{flightStats.distance} NM</strong></div>
              <div>📍 Waypoints: <strong>{flightStats.waypoints}</strong></div>
              {selectedAircraft && (
                <>
                  <div>⏱️ Temps: <strong>{flightStats.time} h</strong></div>
                  <div>⛽ Carburant: <strong>{flightStats.fuel} L</strong></div>
                </>
              )}
            </div>
          </div>
        ...
        {/* Compteur de données (en bas à droite) */}
        {mapReady && (
          <div style={{
            position: 'absolute',
            bottom: '16px',
            ...
            <div style={{ fontWeight: 'bold', marginBottom: '4px' }}>Éléments chargés:</div>
            <div>✈️ {airports?.length || 0} aérodromes</div>
            <div>🔶 {airspaces?.length || 0} espaces aériens</div>
            {showLayers.navaids && <div>📡 {navaids?.length || 0} balises</div>}
            {loading.airports && <div style={{ color: '#f59e0b' }}>⏳ Chargement...</div>}
          </div>
        )}
      </div>
