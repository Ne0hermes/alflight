...
interface AbacGraphWizardProps {
  ...
  /** R6 — wizard RÉDUIT pour l'atelier « image unique » : plus de barre de
   *  sous-étapes ni de pagination de graphes (le focus = clic sur un cadre du
   *  canevas, l'identité vit sous le canevas). Ne rend QUE l'outillage
...
export const AbacGraphWizard: React.FC<AbacGraphWizardProps> = (props) => {
  ...
  const [subStep, setSubStep] = useState<WizardSubStep>(
    atelierMode ? 'curves' : hideImageSubSteps ? 'axes' : 'image'
  );
  ...
  // R6 — en wizard réduit, la SEULE sous-étape rendue est « curves ».
  React.useEffect(() => {
    if (atelierMode && subStep !== 'curves') {
      setEditorMode('idle');
      setCalibrationSession(null);
      setSubStep('curves');
    } else if (!atelierMode && hideImageSubSteps && (subStep === 'image' || subStep === 'position')) {
      setEditorMode('idle');
      setCalibrationSession(null);
      setSubStep('axes');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [atelierMode, hideImageSubSteps, subStep]);
      ...
      case 'image':
        return (
          <div style={{ padding: 16 }}>
            ...
            {/* R6 — identité (rôle + opération canonique) extraite dans GraphIdentityPanel :
                aussi montée par le BUILDER sous le canevas (cette sous-étape est masquée
                dès que l'atelier image unique est actif). */}
            <GraphIdentityPanel graph={graph} onUpdateGraph={onUpdateGraph} />
        ...
        return (
          <div style={{ padding: 16 }}>
            <h3 style={{ marginTop: 0 }}>
              {atelierMode ? '✏️ Courbes du cadre actif' : '✏️ Étape 5 : Tracer les courbes'}
            </h3>
  ...
  // R6 : en wizard réduit il n'y a PLUS de sous-étapes — seul reste le geste
  // de sortie « Interpoler & Valider » (l'unique chemin vers l'étape finale).
  const renderNav = () => {
    if (atelierMode) {
      return (
  ...
  return (
    <div style={{ border: '1px solid var(--border-subtle)', borderRadius: 8, backgroundColor: 'var(--bg-surface)' }}>
      {/* R6 — wizard réduit : plus de pagination de graphes ni de barre de
          sous-étapes en atelier (focus = clic sur un cadre du canevas). */}
      {!atelierMode && renderSubStepBar()}
      {renderSubStep()}
      {renderNav()}
    </div>
  );
