...
const isLintable = (f) => /\.(jsx?|mjs|cjs)$/.test(f) && existsSync(path.join(ROOT, f));
...
function resolveTargets() {
  ...
  if (flags.has('--all')) {
    ...
    return git(['ls-files', 'src/**/*.js', 'src/**/*.jsx']).filter(isLintable);
  }
...
if (!targets.length) {
  console.log('✓ [lint:tokens] Aucun fichier JS/JSX modifié à vérifier.');
  process.exit(0);
}
...
const result = spawnSync(
  process.execPath,
  [ESLINT_BIN, '--no-eslintrc', '-c', CONFIG, ...targets],
  {
    cwd: ROOT,
    stdio: 'inherit',
