...
import { convertValue, getUnitSymbol } from './unitConversions.js';
import { FUEL_DENSITIES as CANON } from './constants.js';
import { getFuelDensity as canonicalFuelDensity } from './fuelDensity.js';
...
export function toStorage(value, userUnit, category, opts = {}) {
  ...
  try {
    ...
    // Silence volontaire : le null retourné EST le signal (règle du paquet).
    return null;
  }
}
...
export function fromStorage(storedValue, userUnit, category, opts = {}) {
  ...
  try {
    ...
    // Silence volontaire : la chaîne vide retournée EST le signal.
    return '';
  }
}
