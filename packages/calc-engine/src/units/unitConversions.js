...
import { FUEL_DENSITIES } from './constants.js';
...
// 🔇 Règle du paquet : aucune sortie console. Une conversion refusée est un
// ÉVÉNEMENT MÉTIER — c'est à l'appelant de décider s'il l'affiche, le
// journalise ou l'ignore. Par défaut : silence (le NaN retourné est déjà le
// signal, et le contrat fail-closed en aval le traite comme une donnée
// manquante).
let reportConversionFailure = null;

/**
 * Branche un rapporteur d'échec de conversion (application, tests, serveur).
 * @param {((message: string) => void)|null} reporter
 */
export function setConversionFailureReporter(reporter) {
  reportConversionFailure = typeof reporter === 'function' ? reporter : null;
}
...
const conversionFailure = (message) => {
  ...
  if (reportConversionFailure) reportConversionFailure(message);
  return NaN;
};
