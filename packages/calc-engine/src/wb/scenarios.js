...
import { normalizeAircraftArmsToMeters } from './armUnits.js';
import { computeScenarioFuel } from './fuelArm.js';
...
export const calculateScenarios = (aircraft, calculations, loads, fobFuel, fuelData, fuelUnit = 'ltr', activeTankIds = null, burnedOverrideLtr = null) => {
  ...
  // Règle du paquet : aucune sortie console. L'absence de données de centrage
  // est signalée par le null retourné — c'est à l'appelant de l'afficher.
  if (!aircraft.weightBalance) return null;
  ...
  // CODE MORT RETIRÉ (Phase 2) : ces quatre recopies du résultat du moteur
  // amont n'étaient lues NULLE PART. Ce moteur recalcule tout depuis l'avion
  // et les charges — le paramètre `calculations` ne sert donc plus qu'à
  // vérifier qu'un calcul amont a bien eu lieu (garde en tête de fonction).
  ...
  // Un dépassement de masse sans carburant est un ÉVÉNEMENT MÉTIER, pas une
  // trace technique : il repart dans le résultat (champ zfwExceededMessage,
  // à côté du booléen déjà exposé) au lieu d'être écrit en console, où le
  // pilote ne le verra jamais et où un moteur serveur n'a rien à écrire.
  const zfwExceededMessage = isZfwExceeded
    ? `Masse sans carburant ${zfwCalc.totalWeight.toFixed(1)} kg supérieure à la limite MZFW (${maxZeroFuelMass.toFixed(1)} kg).`
    : null;
  ...
  return {
    ...
    zfw: {
      ...
      exceededMessage: zfwExceededMessage,
