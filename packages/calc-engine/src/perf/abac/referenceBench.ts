...
export function runReferenceCase(graphs: GraphConfig[], rc: ReferenceCase): ReferenceCaseResult {
  ...
  // 🔇 Phase 2 — ce bloc REMPLAÇAIT temporairement console.log et console.warn
  // par des fonctions vides, le temps du rejeu : le moteur de cascade
  // journalisait tant qu'un banc de cinq cas noyait la console. C'était une
  // MUTATION D'UN OBJET GLOBAL — proscrite dans un moteur de calcul (et
  // franchement dangereuse : une exception hors du bloc `finally` aurait laissé
  // la console muette pour toute l'application).
  // Le moteur étant désormais silencieux par défaut (traceur injectable), le
  // contournement n'a plus lieu d'être.
  const result = performCascadeCalculationWithParameters(chain, rc.inputValue, params);
